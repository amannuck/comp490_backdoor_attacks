...
def transform_to_same_length(x, n_var, max_length):
    ...
    ucr_x = np.zeros((n, max_length, n_var), dtype=float)
...
#                     output_dir = root_dir + '/results/' + classifier_name + '_generator/' \
#                                  + curr_archive_name + '/' + dataset_name + '/' + 'df_metrics.csv'
#                     if not os.path.exists(output_dir):
#                         print(f"❌ Missing file: {output_dir}")
#                         continue
...
def generate_results_csv(output_file_name, root_dir):
    ...
    # Regex for parsing backdoor_save filenames
    pattern = re.compile(r"epoch\d+_(\d+\.\d+)_(\d+\.\d+)_(\d+\.\d+)\.weights\.h5")
        ...
        for archive_name in ARCHIVE_NAMES:
            ...
            for it in range(ITERATIONS):
                ...
                for dataset_name in datasets_dict.keys():
                    ...
                    # 🔹 Extra step: Look inside backdoor_save/ if exists
                    backdoor_dir = root_dir + '/results/' + classifier_name + '_generator/' \
                                   + curr_archive_name + '/' + dataset_name + '/' + 'backdoor_save'

                    if os.path.exists(backdoor_dir):
                        best_asr = -1
                        clean_acc = None
                        train_asr = None
                        for fname in os.listdir(backdoor_dir):
                            match = pattern.match(fname)
                            if match:
                                c_acc = float(match.group(1))
                                t_asr = float(match.group(2))
                                tr_asr = float(match.group(3))
                        ...
                        if best_asr != -1:  # found something
                            df_metrics['clean_acc'] = clean_acc
                            df_metrics['test_asr'] = best_asr
                            df_metrics['train_asr'] = train_asr

                    res = pd.concat((res, df_metrics), axis=0, sort=False)

    # Save raw results with ASR + acc info
    res.to_csv(root_dir + output_file_name, index=False)

    # Aggregate mean accuracy per dataset (same as before)
    res = pd.DataFrame({
        'accuracy': res.groupby(
...
def save_logs(output_directory, hist, y_pred, y_true, duration,
    ...
    if lr == True:
        if 'lr' in row_best_model:
            df_best_model['best_model_learning_rate'] = row_best_model['lr']
        elif 'learning_rate' in row_best_model:
            df_best_model['best_model_learning_rate'] = row_best_model['learning_rate']
        else:
            df_best_model['best_model_learning_rate'] = None
